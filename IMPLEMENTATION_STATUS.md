...
- ✅ Transactional email (welcome, named-invitation notification, email-invitation join link, password-reset) sends via Resend, live in production. Content lives in `crates/server-game/emails/*.txt` (currently placeholder wording). See `docs/authentication.md`'s status section.
