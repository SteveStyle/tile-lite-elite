...
- ✅ Login/register UI exists (web + desktop), with "Remember me" (prefills display name only) and "Stay logged in" checkboxes. "Stay logged in" is now purely client-side: it controls whether the browser persists the session token across a restart. It **no longer** sets the server-side lifetime — both session limits below are uniform for every session.
- ✅ **Sessions now actually expire**, on two uniform limits (no longer tied to "Stay logged in"): an **idle window** — a session unused for longer than `SESSION_IDLE_WINDOW_SECS` (48h) is rejected and pruned — and an **absolute cap** — every session is set to expire `SESSION_MAX_LIFETIME_SECS` (10 days) after creation, never extended, so even a continuously-active session must re-authenticate eventually. `last_seen_at` (bumped, throttled, per authenticated request) drives the idle check; `expires_at` (set once at login) the absolute one. **Explicit logout** (`POST /auth/logout`) deletes the session outright — acting on certainty rather than guessing from inactivity. Expired rows are also swept lazily whenever `GET /games` runs (`delete_expired_sessions`), same pattern as the move-time-limit/finished-game cleanup (no background scheduler). `stay_logged_in` is kept only as a record of the client-side choice. See `2.7-authentication-and-invitations.md`'s login notes and `4.2-database-schema.md`'s sessions table for the full reasoning.
