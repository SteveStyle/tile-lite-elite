...
**2. Refresh the dev environment** — rebuild and restart so you're testing *this* code, not a process still running an old binary. `services.sh restart-server` kills whatever is bound to `:3000` — even a hand-started or orphaned process its own PID file doesn't know about — and starts a fresh `cargo run`; use `restart` instead to refresh the web client too.

```bash
./scripts/services.sh restart-server      # or: restart (also refreshes the web client)
```
...
```bash
cargo test --workspace
# Playwright UI suite: <command TBD — suite being established>
```

CI runs `fmt`/`clippy`/`cargo test`/wasm on the push (step 1) too; running them here first is just the faster local loop.
