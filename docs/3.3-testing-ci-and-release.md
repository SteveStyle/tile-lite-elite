...
of your working tree; the plain build in step 3 is what needs the commit from
step 1.) When you fix something found in staging and rebuild, a plain step-3
...
If anything is wrong, fix it, commit again (step 1), and rebuild (step 3) — re-seeding (step 2) again only if you're re-testing a migration. **Do not run
`verify` yet** — staging is ahead of production at this point, so a mismatch is
expected, not an error.

**5. Confirm HEAD is pushed** — `deploy.sh` requires it. If you pushed each commit at step 1 (you should have), this is already done; run it again to be sure the final fix commit is up:
...
sequenceDiagram
    ...
    Dev->>GH: 5. confirm final commit pushed
