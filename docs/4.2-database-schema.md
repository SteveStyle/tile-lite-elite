...
- `created_at` text not null
- `last_seen_at` text not null — set once at creation; `update_session_last_seen` exists in `persistence.rs` to bump it but **has no call sites yet** — in practice this value doesn't change after the session is created today. It is kept deliberately (unlike the columns dropped in migration `0003`): the plan is to wire it up so idle-session cleanup can prune `stay_logged_in` sessions that have no `expires_at` to fall back on.
...
Two migrations follow the baseline: `0002_player_ratings_and_stats.sql` (the rating/stats tables and columns) and `0003_drop_dead_columns.sql`, which drops the columns that were only ever written with a placeholder value and read by nothing — `games.notes`, `game_participants.left_at`, `game_moves.tiles_json`, `game_moves.is_validated`, and `sessions.game_id`. (`last_seen_at` on `players`/`sessions` is deliberately **not** dropped — it is read/displayed and slated to drive idle-session cleanup.)
