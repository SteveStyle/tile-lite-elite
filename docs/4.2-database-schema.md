...
- `display_name` text not null, **unique — case-insensitively** (the column's original case-sensitive `unique` plus `idx_players_display_name_nocase`, a `unique … collate nocase` index added by `0005`; two players can't register names differing only by case). Stored and displayed exactly as entered at registration — only *matching* is case-folded, so login resolves "Alice"/"alice"/"ALICE" to the one account (see `persistence::get_player_by_name`). NOCASE folds ASCII A–Z only.
