...
erDiagram
    ...
    games |o--o{ sessions : "game_id (nullable)"
    ...
    sessions {
        text id PK
        text player_id FK
        text game_id FK "nullable"
        text expires_at "nullable = no expiry"
    }
...
- `status` text not null — `waiting`, `active`, or `finished`. Mirrors `GameSession.status`, rewritten on every `save_game` call.
...
- `notes` text null — **dead column**: always written as `NULL` in `save_game`; nothing in the codebase sets or reads it otherwise.
...
- `left_at` text null — **dead column**: always written as `NULL`; nothing sets it (a departed/removed participant is represented by `player_id` going back to `null` or `snapshot_json`'s `removed_by_player` flag, not this column).
...
- `tiles_json` text null — **dead column**: always written as `NULL`; the actual tile placements live inside `payload_json` instead.
- `payload_json` text not null — the full serialized `MoveRecord` (move type, tiles placed, score delta, human-readable description, etc.) — this, not `tiles_json`/`score_delta`, is what `MoveRecordDto` is actually built from on read.
...
- `is_validated` integer not null default 1 — **effectively dead**: the INSERT hardcodes the literal `1` for every row; no code path ever writes anything else, so this column carries no real information today.
...
- `game_id` text null — **dead column in practice**: declared in the schema and referenced in `4.2 Database Schema`'s own Suggested Relationships as "a session may point to a game," but `create_session`'s `insert` never binds it — it's always `NULL`. No per-game session/presence tracking is actually implemented.
...
- `created_at` text not null
- `last_seen_at` text not null — set once at creation; `update_session_last_seen` exists in `persistence.rs` to bump it but **has no call sites anywhere** — in practice this value never changes after the session is created, despite the column name implying ongoing presence tracking.
