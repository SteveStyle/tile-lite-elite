...
**2. Seed staging with production's current state** (schema changes only) — wipes staging and rebuilds it at whatever commit prod is running, so the migration in step 3 applies to production's actual pre-migration schema, not a volume that already has it:
