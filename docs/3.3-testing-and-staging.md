...
rehearsal; steps 5–7 ship it. For a change **with a new migration**, do step 1
so the migration is exercised against production-like data; for a change with
**no migration**, skip step 1 and let staging reuse whatever data it already
has.

**1. Seed staging with production's current state** (schema changes only) — wipes staging and rebuilds it at whatever commit prod is running:

```bash
./scripts/deploy-staging.sh at prod
```

**2. Commit the change** — every build below uses a committed HEAD, never a dirty tree:
...
The order of steps 1 and 2 doesn't matter: `at prod` builds production's commit
in a throwaway worktree, independent of your local tree.

**3. Build your commit onto the staging volume** — applies any new migration on top of the step-1 data (a plain run does not wipe the volume):
...
If anything is wrong, fix it, return to step 2, and rerun step 3. **Do not run
