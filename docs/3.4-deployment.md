...
`deploy.sh` refuses to run unless two things are both true: the working tree is clean (it only ever builds a real commit, never uncommitted changes — commit or stash first), and [local staging](3.3-testing-and-staging.md#staging-environment) is currently up and running *that exact commit* (checked via its `/health`). The second check exists specifically to catch the easy-to-miss case: test commit A in staging, then commit a "quick fix" B before running `deploy.sh` — without this, B would ship to production having never been tested anywhere. Bring staging to HEAD first (`./scripts/deploy-staging.sh`) if either check fails.
