...
The production-side machinery a release ships onto: the container topology, the
two images, secrets, and what `deploy.sh` does under the hood. The end-to-end
release *procedure* (test → stage → push → deploy → verify, with the command
runbook and pipeline diagram) lives in
[Testing, CI & Release](3.3-testing-ci-and-release.md#shipping-a-change-the-full-sequence);
this doc is the reference that procedure points into, not a second copy of it.
Part of the lifecycle series — see [docs/README.md](README.md) for the full
sequence. Follows [Testing, CI & Release](3.3-testing-ci-and-release.md),
precedes [Production Support & Maintenance](3.5-production-support.md).
...
`deploy.sh` is step 6 of the release runbook in [Testing, CI & Release](3.3-testing-ci-and-release.md#shipping-a-change-the-full-sequence) — this is what it does under the hood. The live VM has 1GB RAM — not enough to compile the Rust/wasm workspace — so images are always built locally and shipped over, never built on the VM itself:

```bash
./scripts/deploy.sh
```

`deploy.sh` refuses to run unless two things are both true: the working tree is clean (it only ever builds a real commit, never uncommitted changes — commit or stash first), and [local staging](3.3-testing-ci-and-release.md#staging-environment) is currently up and running *that exact commit* (checked via its `/health`). The second check exists specifically to catch the easy-to-miss case: test commit A in staging, then commit a "quick fix" B before running `deploy.sh` — without this, B would ship to production having never been tested anywhere. Bring staging to HEAD first (`./scripts/deploy-staging.sh`) if either check fails.
