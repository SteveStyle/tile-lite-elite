...
(Setup → Development → Testing & Staging → Deployment → Production
...
Plain `http://` requests get redirected to `https://` automatically — no separate config for that either. (Staging skips all of this — see [Testing & Staging](3.3-testing-and-staging.md#staging-environment).)
