...
Creating a game requires being signed in — every Human seat needs a `claim` (`creator`/`named`/`open`/`email`; see `2.7-authentication-and-invitations.md` for the full model), and there's no more "anonymous" game creation.
...
Bob's invitation was created — and, with a mail provider configured (see `2.5-authentication.md`'s status section), emailed to him — in this same request. Save the `id` (e.g. `game-123-abc`).
...
Note: The `player_id` is the same, but the `session_token` is new — and the old one is *not* invalidated by logging in elsewhere (there's no "log out other devices" concept beyond a self-service password change, which invalidates all of them). `stay_logged_in` here only controls whether the client persists the token across a restart; every session expires on the same two server-side limits regardless — a 48h idle window and a 10-day absolute cap — cleaned up automatically, or immediately on an explicit `POST /auth/logout`. See `2.7-authentication-and-invitations.md`'s login notes for the full reasoning. Store the new token securely.
