...
- ✅ **Transactional email** (added 2026-07-17): `crates/server-game/src/email.rs` sends via Resend — welcome (on register), an invitation notification (named invitations only; open/stranger invitations have no specific invitee to notify), and the forgot-password link above. Content lives in `crates/server-game/emails/*.txt` (plain text, `{{placeholder}}` substitution, no templating engine) — edit the wording there, not in Rust. `RESEND_API_KEY` unset (the default) means every send just logs instead, so nothing here needs a live provider to work or to test. **What's missing**: the Resend account/domain itself isn't set up yet (see `docs/operations.md`'s Configuration table and Container Deployment section for what that involves), so nothing has actually been emailed for real yet — only logged.
