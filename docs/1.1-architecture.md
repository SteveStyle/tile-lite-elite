...
flowchart LR
    ...
    subgraph StagingEnv["Staging (local, ephemeral)"]
        StagingWeb["web (Caddy, :8081, HTTP)"] --> StagingServer["server"]
        StagingServer --> StagingDB[("tile-lite-elite-staging-data")]
    end
    ...
    Build -- "docker build (same Dockerfile)" --> StagingEnv
    Build -- "docker save / scp / docker load\n(scripts/deploy.sh)" --> ProdVM
    StagingEnv -. "verified clean, then" .-> ProdVM
```

The same `Dockerfile` produces every image in every environment — staging and production never diverge in what's built, only in how it's configured (`docker-compose.staging.yml` vs `docker-compose.yml`) and where it runs. See [3.3 Testing, CI & Release](3.3-testing-ci-and-release.md) and [3.4 Deployment](3.4-deployment.md) for the actual commands.
