...
# admin.sh — Build (release) and run the admin CLI (`scrabble-admin`,
...
# connection, by design (see docs/operations.md's "Admin CLI" section).
# That means this script can't reach the Oracle VM's server; use
# `docker compose exec server scrabble-admin ...` there instead (same doc
# section covers it).
#
# Usage:
...
exec ./target/release/scrabble-admin "$@"
